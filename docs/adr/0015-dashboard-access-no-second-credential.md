...
- **Residual, stated plainly:** Grafana OSS **requires** an admin user; it cannot
  ...
  a random 32-char value delivered via `admin.existingSecret`. (Originally
  generated by the `argocd` stack; now sourced from AWS Secrets Manager via
  External Secrets, see [ADR-0016](0016-platform-secrets-external-secrets.md).) It is never in Git and never surfaced; it does live
